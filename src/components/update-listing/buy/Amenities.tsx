...
const Amenities: FC<PageAddListing4Props> = ({
  ...
  const [newAmenities, setNewAmenities] = useState<Feature[]>(
    formData.amenities || []
  );

  const amenities = newAmenities.map(({ name, icon }) => ({ name, icon }));
  ...
  const handleCheckboxChange = (
    ...
    if (checked) {
      setNewAmenities((prevAmenities) => [...prevAmenities, { name, icon }]);
    } else {
    ...
    <>
      ...
      <div className="flex justify-between">
        ...
        <div className="space-x-4 ">
          <button
            type="button"
            className="ttnc-ButtonSecondary px-6 py-3 rounded-full active:scale-90 transition duration-150 font-medium border bg-white border-neutral-200 text-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800"
